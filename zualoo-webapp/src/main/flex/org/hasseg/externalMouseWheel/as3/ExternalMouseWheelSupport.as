package org.hasseg.externalMouseWheel.as3 {
	...
	import org.hasseg.externalMouseWheel.ExternalMouseWheelEvent;
	...
	*
	* @author Pavel fljot ( http://inreflected.com ) - a couple of changes + pure AS3-projects support:
	* 1. initialization by getInstance(stage:Stage) method, that gets reference to the Stage;
	* 2. getAllInteractiveObjectsOnStage() is renamed to getActuallyMouseEnabledInteractiveObjectsOnStage()
	...
	*    (for same purposes as getActuallyMouseEnabledInteractiveObjectsOnStage() ).
	*/
	public final class ExternalMouseWheelSupport {
		...
		*/
		public static function getInstance(stage:Stage):ExternalMouseWheelSupport {
			_stage = stage;
		...
		private function externalMouseWheelEventHandler(aDelta:int, aX:Number, aY:Number):void {
			...
			if (registeredObjects.length > 0) {
				...
				var hitTestObjs:Array = rObjs.filter(function(aItem:*, aIndex:int, aArray:Array):Boolean {
					return (aItem.object.hitTestPoint(aX,aY));
				});
		...
		private function getRegisteredObjects():Array {
			if (_registerAutomatically == true) {
				_automaticallyRegisteredObjects = getActuallyMouseEnabledInteractiveObjectsOnStage();
				return _automaticallyRegisteredObjects;
			}else{
				var filteredManuallyRegisteredObjects:Array = _manuallyRegisteredObjects.filter(function(aObj:*, aIndex:int, aArray:Array):Boolean {
					return objectIsActuallyMouseEnabled(aObj);
				});
				return filteredManuallyRegisteredObjects;
			}
		}
		...
		private function getActuallyMouseEnabledInteractiveObjectsOnStage():Array {
				...
				function travelDisplayList(container:DisplayObjectContainer):Array {
					...
					for (var i:uint=0; i < container.numChildren; i++) {
						...
						if (child is InteractiveObject) {
							...
							// get children only if they are mouse-enabled. otherwise no need to scan them
		...
		private function objectIsActuallyMouseEnabled(aObj:InteractiveObject, checkMouseEnabledProperty:Boolean = true):Boolean {
			
			var isEnabled:Boolean = true;
			
			if (!aObj.visible || (checkMouseEnabledProperty && !aObj.mouseEnabled)) {
				isEnabled = false;
			}
			
			if (isEnabled && aObj.parent) {
				// need to check parents if they are visible
				if (!(aObj.parent as DisplayObjectContainer).mouseChildren) {
					isEnabled = false;
				} else {
					isEnabled = objectIsActuallyMouseEnabled(aObj.parent, false);
				}
			}
